...
describe('Group summary utils', () => {
  test('calculateGroupSummary - should calculate correct main summary for given data', () => {
    expect(calculateGroupSummary({ groupData: null, stocksData: SUMMARY_PLUS_RESULT_DATA.stockData })).toEqual(
      SUMMARY_PLUS_RESULT_DATA.mainSummary,
    );

    expect(calculateGroupSummary({ groupData: null, stocksData: SUMMARY_MINUS_RESULT_DATA.stockData })).toEqual(
      SUMMARY_MINUS_RESULT_DATA.mainSummary,
    );
  });
